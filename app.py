...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    st.markdown("*Evaluated using Stratified 5-Fold Cross-Validation + Leave-One-Out CV*")
    ...
    with col1:
        st.markdown("""<div class="metric-card">
            ...
            <div class="metric-label">Winning model</div></div>""",
            unsafe_allow_html=True)
    with col2:
        st.markdown("""<div class="metric-card">
            <div class="metric-value">0.947</div>
            <div class="metric-label">RF Macro F1 (5-Fold CV)</div></div>""",
            unsafe_allow_html=True)
    with col3:
        st.markdown("""<div class="metric-card">
            <div class="metric-value" style="color:#1D9E75">0.951</div>
            <div class="metric-label">RF LOO F1 (Leave-One-Out)</div></div>""",
            unsafe_allow_html=True)
    ...
    results = {
        "Model":          ["KNN (K=9, distance)", "Random Forest"],
        "F1 — High risk": [0.909, 0.933],
        "F1 — Low risk":  [0.939, 0.962],
        "Precision":      [0.917, 0.947],
        "Recall":         [0.942, 0.947],
        "Accuracy":       [0.927, 0.951],
        "Macro F1":       [0.924, 0.947],
        "LOO F1":         [0.878, 0.951],
    }
    ...
    st.caption(
        "5-Fold CV: dataset split into 5 folds, tested on each — scores averaged. "
        "LOO: each county tested individually. Both confirm RF as winner."
    )

    # ── Experiment comparison table ────────────────────────────────────────────
    st.markdown("### Validation method comparison")
    df_val = pd.DataFrame({
        "Validation method": ["5-Fold CV", "Leave-One-Out CV"],
        "KNN (K=9) F1":      [0.924, 0.878],
        "Random Forest F1":  [0.947, 0.951],
        "Winner":            ["Random Forest", "Random Forest"],
    })
    st.dataframe(df_val, use_container_width=True, hide_index=True)
    st.caption("Both validation methods consistently identify Random Forest as the superior model.")
    ...
    b1 = ax.bar(x - w/2, knn_vals, w, label="KNN (K=9, distance)", color="#9b5de5", edgecolor="#333")
    ...
    ax.set_title("KNN (K=9) vs Random Forest — 5-Fold CV Performance",
                 color=TEXT_CLR, fontsize=12)
    ...
    st.info("""
**Random Forest outperformed KNN (K=9) across all metrics
(Macro F1 = 0.947 vs 0.924).**
...
KNN still performed strongly (F1 = 0.924), confirming that geographic
proximity is a powerful signal for GBV risk. Its lower LOO score (0.878
vs 0.951) suggests Random Forest generalises more consistently when
tested on individual unseen counties.

The **F1 of 0.962 for low-risk counties** means the model correctly
identifies safe counties too — reducing unnecessary resource allocation.
    ...
    # ── Methodology note ───────────────────────────────────────────────────────
    st.success("""
✅ **Validation approach — why cross-validation?**
...
giving a more reliable estimate. Leave-One-Out CV tests every county
individually — the most conservative possible evaluation.
Both methods consistently confirm Random Forest as the stronger model.
    """)
